...
def streamplot(axes, x, y, u, v, density=1, linewidth=None, color=None,
               ...
               maxlength=4.0, integration_direction='both', num_arrow_heads=2):
    ...
    *integration_direction* : ['forward', 'backward', 'both']
    ...
    if start_points is None:
        mask_options = [StreamMask(density),  StreamMask(density)]
        starting_points_options = [_gen_starting_points_CCW(mask.shape),
                                   _gen_starting_points_CW(mask.shape)]
        trajectories_options = [[], []]
        num_points = [[], []]
        for starting_points, trajectories_temp, num_points_temp, mask_temp in \
                zip(starting_points_options, trajectories_options, num_points, mask_options): 
            dmap_temp = DomainMap(grid, mask_temp)
            integrate_temp = get_integrator(u, v, dmap_temp, minlength, maxlength,
                                            integration_direction)
            for xm, ym in starting_points:
                if mask_temp[ym, xm] == 0:
                    xg, yg = dmap_temp.mask2grid(xm, ym)
                    t = integrate_temp(xg, yg)
                    if t is not None:
                        trajectories_temp.append(t)
            num_points_temp.append(np.sum([np.size(traj, axis=1)
                                           for traj in trajectories_temp]))
        if num_points[0] <= num_points[1]:
            trajectories = trajectories_options[0]
        else:
            trajectories = trajectories_options[1]
        del trajectories_options
        del starting_points_options
    else:
    ...
    for t in trajectories:
        ...
        fractions = np.linspace(0, 1, num_arrow_heads+2)
        fractions = fractions[1:-1]
        for frac in fractions:
            s = np.cumsum(np.sqrt(np.diff(tx) ** 2 + np.diff(ty) ** 2))
            n = np.searchsorted(s, s[-1]*frac)
    ...
    return stream_container, mask, dmap, t
...
def _gen_starting_points_CCW(shape):
...
def _gen_starting_points_CW(shape):
    """Yield starting points for streamlines.
