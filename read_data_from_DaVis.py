...
def read_3C_velocity(num_trials, num_snaps, num_vects, RAWdirs):
    skiprows = 3
    usecols = [3, 4, 5]
...
def read_2D_geometry(num_trials, num_snaps, num_vects, RAWdirs):
    skiprows = 3
    usecols = [0, 1]
...
def read_geometry_and_velocity(num_trials, num_snaps, RAWdirs,
                               flowdir='downward', measurments='2D,3C'):
    if measurments == '2D,3C':
        if flowdir == 'downward':
            x_g, y_g, num_x, num_y, num_vects \
                = read_2D_geometry_downwards_flow(num_trials, num_snaps,
                                                  RAWdirs)
            u_g, v_g, w_g = read_3C_velocity_downwards_flow(
                    num_trials, num_snaps, num_x, num_y, num_vects, RAWdirs)
        elif flowdir == 'right':
            print 'add functionality here'
    elif measurments == '2D,2C':
        print 'add functionality here'
